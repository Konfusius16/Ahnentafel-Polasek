...
function initDefaults(){
  for (const wrap of document.querySelectorAll('[data-stem]')){
    ...
    let radio = wrap.querySelector('input[value=svg]');
    if (!radio) radio = wrap.querySelector('input[type=radio]');
    if (radio){ radio.checked = true; applyChoice(stem, radio.value); }
  }
}
...
def format_group_html(stem, fmt_map):
    ...
    # Prefer svg as default, then pdf, then png
    for preferred in (".svg", ".pdf", ".png"):
        if preferred in fmt_map:
            first_ext = preferred.lstrip(".")
            break
    return (
